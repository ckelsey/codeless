...
import Get from "../src/utils/objects/get.js"
import Try from "../src/utils/try.js"

const transformRequire = (_program: ts.Program) => transformerFactory
...
function visitSourceFile(sourceFile: ts.SourceFile, context: ts.TransformationContext) {
    ...
    function declarationReducer(result: boolean, declaration: any) {
        if (result !== true) {
            result = Try(() => Get(declaration, 'initializer.expression.getText()')) === 'require'
        }
        return result
    }
