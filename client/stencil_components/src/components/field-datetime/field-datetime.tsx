...
export class FieldDatetime {
    ...
    @Listen('timechange') onTimeChange() {
        ...
        if (this.value.toString() !== newDate.toString()) { this.value = newDate }
    }
    ...
    timeElement!: HTMLFieldTimeElement
    ...
    isempty() { return this.value === undefined || invalidDate(this.value) }
    ...
    updateDate(newVal) {
        ...
        const localOptions: any = { year: 'numeric', month: 'long', day: 'numeric', hour12: true, hour: 'numeric', minute: 'numeric' }

        if (this.showseconds) {
            localOptions.second = 'numeric'
        }

        this.startDate = date
        this.localeString = date.toLocaleString(undefined, localOptions)
        this.monthString = date.toLocaleString(undefined, { month: 'long' })
        this.yearString = date.toLocaleString(undefined, { year: 'numeric' })
        this.time = date.toLocaleTimeString(undefined, { hour12: false })
    }
    ...
    render() {
        this.formInput = RenderLightDom(this.host, 'input.field-datetime-hidden-input', {
            tagName: 'input',
            type: 'text',
            value: this.value,
            name: this.name,
            ...
            class: 'field-datetime-hidden-input',
            slot: 'form-control'
        }) as HTMLInputElement
        ...
        >
            ...
            <div class="field-input-label">
                <drop-down
                    ...
                    class="field-datetime-drop-down"
                    closeonclick={false}
                    openonhover={false}
                    arrow={false}
                >
                    <div class="field-datetime-input" slot="label">{this.localeString}</div>
                    <div class="field-datetime-overlay" slot="item">
                        <calendar-header date={this.value}></calendar-header>
                        <calendar-month date={this.startDate}></calendar-month>
                        <field-time
                            value={this.time}
                ...
                <label ref={(el) => this.labelElement = el as HTMLLabelElement} class="field-datetime-label">{this.sanitizedLabel}</label>
            </div>
